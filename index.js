...
function validateConfig(config) {
  try {
    ...
    assert.ok(name, "No 'name' on Kinder configuration object.".red);

    assert.ok(commands, "No 'commands' objedt on Kinder configuration object.".red);
    assert.ok(commands.default, "No 'default' command on Kinder 'commands' config object.".red);
    ...
    print(`Kinder configuration is invalid: ${e.message}`.red);
    return false;
  }
}
...
function formatText(text, maxLength, char = " ", align = "left") {
  ...
  if (truncatedString.length < maxLength) {
    if (align == "left") {
      return truncatedString + char.repeat(maxLength - truncatedString.length);
    } else {
      return char.repeat(maxLength - truncatedString.length) + truncatedString.length;
    }
  } else {
